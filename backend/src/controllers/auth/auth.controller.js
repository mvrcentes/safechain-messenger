...
export const logout = async (req, res) => {
  ...
  if (!token) {
    console.log(chalk.yellow(" No refresh token found in cookies"))
...
export const login = async (req, res) => {
  ...
  try {
    ...
    if (!password || typeof password !== 'string' || password.length < 8) {
      console.log(chalk.yellow(' Login attempt with short or invalid password'))
...
export const refreshToken = async (req, res) => {
  ...
  if (!user) {
    console.warn(
      chalk.yellow(" Refresh valid session, but user not found:"),
      session.userId
    )
