...
async function handleInit(msg, ws) {
...
async function handleDirectMessage(msg) {
  ...
  const saved = await persistDirectMessage({ prisma, from: msg.from, to: msg.to, content: cleanContent })
...
async function handleGroupMessage(msg) {
  ...
  const cleanContent = sanitizeContent(msg.content)
  const saved = await persistGroupMessage({ prisma, from: msg.from, groupId: msg.groupId, content: cleanContent })
  const group = await prisma.group.findUnique({ where: { id: msg.groupId }, include: { members: true } })
  ...
  wss.on("connection", (ws) => {
    ...
    ws.on("message", async (data) => {
      ...
      try {
        ...
        // unknown types are ignored silently (fail-safe)
      } catch (error) {
